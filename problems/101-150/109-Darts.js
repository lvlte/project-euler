 ...
 * Problem 109 - Darts
 *
 * @see {@link https://projecteuler.net/problem=109}
 *
...
this.solve = function () {
  ...
  // Simple problem, simple solution (though not very efficient) ...

  const limit = 100;

  // Build S, the set of possible outcomes for a single dart.
  const sections = range(1, 21).flatMap(n => ['S'+n, 'D'+n, 'T'+n]);
  const bull = ['S25', 'D25'];
  const S = ['0', ...sections, ...bull];

  // Helpers for scoring
  const sMap = {S:1, D:2, T:3};

  const score = (darts) => {
    let s = 0;
    for (const dart of darts) {
      if (dart === '0') continue;
      s += sMap[dart[0]] * +dart.slice(1);
    }
    return s;
  };

  // Create multisubsets of size 3 from S (k-combinations with repetitions).
  const combi = nkCombinations(S, 3, true);

  // We got an "unordered" checkout for each combination having at least one
  // double. If its score is less than limit, we count one distinct checkout
  // per distinct double in the subset, since it is considered different to
  // finish on one rather than another.

  let nWays = 0;
  for (const darts of combi) {
    const doubles = darts.filter(dart => dart[0] === 'D');
    if (doubles.length > 0 && score(darts) < limit) {
      nWays += new Set(doubles).size;
    }
  }
