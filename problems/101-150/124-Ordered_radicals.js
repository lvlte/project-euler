 ...
 *
 * @see {@link https://projecteuler.net/problem=124}
 *
...
const { rad } = require('../../lib/prime');

this.solve = function () {

  // Nothing fancy here. Again, we leverage the integer keys ordering of JS
  // objects (ascending).

  const sortedRadicals = {};

  for (let n=1; n<=100000; n++) {
    const r = rad(n);
    if (r in sortedRadicals)
      sortedRadicals[r].push(n);
    else
      sortedRadicals[r] = [n];
  }

  const E = [, ...Object.values(sortedRadicals).flat()];
