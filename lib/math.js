...
module.exports = math = {
  ...
  fib(n) {
    ...
    // Create or reuse internal cache object.
    ...
    // F[n] maps n to f(n). We decide whether or not to use BigInt based on the
    // value of n, knowing that f(79) > Number.MAX_SAFE_INTEGER.
   ...
   */
  factorial(n) {
    const f = typeof n === 'bigint' || n > 18 ? math.bigFact : n => n<2 ? 1: n*f(n-1);
    return f(n);
  },
  ...
  binomialCoef(n, k) {
    ...
    return math.mfact(n) / (math.mfact(k)*math.mfact(n-k));
  },
  ...
  isSquare(n) {
    ...
    return math.sqrtBig(n)**2n === BigInt(n)
  },
   ...
   */
  sigma0: memoize(n => math.divisors(n).length),

  /**
   * Memoized version of sigmaDiv(1, n), which returns the sum of the divisors
   * of n, including 1 and n.
   */
  sigma1: memoize(n => math.sum(math.divisors(n))),
