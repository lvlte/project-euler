...
module.exports = _Math = {
  ...
  divisors(n, proper=false) {
    if (n < 0)
      return _Math.divisors(-n, proper);
  ...
  /**
   * Solves the quadratic equation ax² + bx + c = 0 for x ∈ ℝ
   *
   * @param {number} a
   * @param {number} b
   * @param {number} c
   ...
   */
  sigmaDiv(k, n) {
    return _Math.sum(_Math.divisors(n).map(n => n**k));
  },
   ...
   */
  sigma0: memoize(n => _Math.divisors(n).length),

  /**
   * Memoized version of sigmaDiv(1, n), which returns the sum of the divisors
   * of n, including 1 and n.
   */
  sigma1: memoize(n => _Math.sum(_Math.divisors(n))),
   ...
   */
  addF([a, b], [c, d]) {
    return b == d ? _Math.fraction(a+c, b) : _Math.fraction(a*d+c*b, b*d);
  },
   ...
   */
  logb(x, b, check=true) {
    let y = Math.log(x) * (_Math.baseLogRecip[b] || 1/Math.log(b));
    if (check && b**y !== x) {
  ...
  nthRoot(x, n) {
    if (x < 0) {
      x = Math.abs(x);
      return n%2 ? -_Math.nthRoot(x, n) : _Math.nthRoot(x, n);
    }
  ...
  modPow(base, exp, m) {
    ...
    let r = 1;
    for (let i=1; i<=exp; i++)
      r = (r*(base%m))%m; // <- (X)%m each time so that r stays "small"
